...
class CopyButton(Keyboard):
    def __init__(self, text: Text, id: str, copy_text: str | TextWidget, when: WhenCondition = None):
        self.text = text
        self.copy_text = copy_text
        super().__init__(id, when)

    async def _render_keyboard(self, data: dict, manager: DialogManager) -> RawKeyboard:
        text = self.copy_text if isinstance(self.copy_text, str) else await self.copy_text.render_text(data, manager)
        return [
            [
